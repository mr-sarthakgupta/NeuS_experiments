...
class Dataset:
    def __init__(self, conf):
        ...
        # self.learn_camera_params = conf.get_string('learn_camera_params')
        #if (conf.get_string('learn_camera_params') == True):
            # Set up focal & pose networks
            #LearnPose()...
        #else:
        ...
        # self.pose_net = LearnPose(num_cams = self.n_images, learn_R = 0.001, learn_t = True, init_c2w = None)
        ...
        # print('intrinsic_all')
        # print(self.intrinsics_all[0].shape)
    ...
    def get_pose(self):
        ...
        for scale_mat, world_mat in zip(self.scale_mats_np, self.world_mats_np):
            ...
            k = k + 1
        return torch.from_numpy(self.poses_all)
    ...
    def gen_random_rays_at(self, img_idx, batch_size):
        ...
        p = torch.matmul(self.intrinsics_all_inv[0, None, :3, :3], p[:, :, None]).squeeze() # batch_size, 3
        rays_v = p / torch.linalg.norm(p, ord=2, dim=-1, keepdim=True)    # batch_size, 3
        rays_v = torch.matmul(self.pose_all[0, None, :3, :3], rays_v[:, :, None]).squeeze()  # batch_size, 3
        rays_o = self.pose_all[0, None, :3, 3].expand(rays_v.shape) # batch_size, 3
