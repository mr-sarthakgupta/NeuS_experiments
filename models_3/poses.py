...
class LearnPose(nn.Module):
    def __init__(self, num_cams, dataset, init_c2w=None):
        """
        :param num_cams:
        ...
        self.og_init_c2w = dataset.get_pose()
        if init_c2w is not None:
            self.init_c2w = nn.Parameter(init_c2w, requires_grad=True)
    ...
    def forward(self, cam_id):
        ...
        if self.init_c2w is not None:
            print(f'c2w matrix sum: {torch.linalg.norm(self.init_c2w - self.og_init_c2w.to(self.device))}')
