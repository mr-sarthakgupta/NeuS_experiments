...
from models.datasetmm import Dataset
...
class Runner:
    def __init__(self, conf_path, mode='train', case='CASE_NAME', is_continue=False):
        ...
        self.pose_net = LearnPose(num_cams = self.dataset.get_num_cams(), init_c2w = self.dataset.get_pose().to(self.device).type(torch.float32) + 5*torch.randn(size = self.dataset.get_pose().shape).to(self.device).type(torch.float32), dataset = self.dataset)
